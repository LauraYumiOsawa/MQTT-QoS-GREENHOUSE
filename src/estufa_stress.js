...
function imprimirTabela(fase) {
  ...
  console.log(`\n╔═════════════════════════════════════════════════════════════════════╗`);
  console.log(`║  TABELA COMPARATIVA — ${fase.padEnd(46)}║`);
  console.log(`╠════════════════╦═════╦══════════╦═══════════╦══════════╦════════════╣`);
  console.log(`║ Sensor         ║ QoS ║ Enviadas ║ Recebidas ║ Perdidas ║ Duplicadas ║`);
  console.log(`╠════════════════╬═════╬══════════╬═══════════╬══════════╬════════════╣`);

  function linha(nome, qos, env, s) {
    const recebidasUnicas = Math.max(0, s.recebidas - s.duplicadas);
    const perdidas = Math.max(0, env - recebidasUnicas);
    console.log(
      `║ ${nome.padEnd(14)} ║  ${qos}  ║ ${String(env).padStart(8)} ║ ${String(s.recebidas).padStart(9)} ║ ${String(perdidas).padStart(8)} ║ ${String(s.duplicadas).padStart(10)} ║`
    );
  }
...
// Fase 1: Subscriber ativo por 20 s
let subscriber = criarSubscriber();
console.log("\n=== FASE 1: Subscriber ONLINE por 20 s ===\n");

setTimeout(() => {
  console.log("\n=== FASE 2: Simulando FALHA DE REDE — Subscriber desconectado por 20 s ===");
  ...
  // Fase 2: offline por 20 s — publisher continua
  setTimeout(() => {
    console.log("\n\n=== FASE 3: Subscriber RECONECTADO — coletando mensagens retidas ===\n");
    subscriber = criarSubscriber();

    // Fase 3: aguarda 20 s recebendo mensagens retidas, depois imprime relatório final
    setTimeout(() => {
